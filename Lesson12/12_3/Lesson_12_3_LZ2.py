...
class Bus:
    ...
    def board_passenger(self, passenger_name, boarding_stop, exit_stop):
        ...
        for seat, passenger in self.seats.items():
            if passenger is None and seat not in self.reserved_seats:
                ...
                print(f"Пассажир {passenger_name} купил билет заранее на остановку {boarding_stop}. "
                      f"Место {seat} зарезервировано (синее).")
                break
    ...
    def passenger_without_ticket(self, passenger_name, boarding_stop, exit_stop):
        ...
        # Флаг для отслеживания успешной посадки
        seated = False
        ...
        # Если пассажир не сел, добавляем его в список отказанных
        if not seated:
            print(f"{passenger_name} не смог сесть в автобус на остановке {boarding_stop} — мест нет.")
            self.rejected_passengers.append((passenger_name, boarding_stop, exit_stop))

        # Обновление флага наличия свободных мест
        if all(self.seats[seat] is not None for seat in self.seats):
            self.free_seats_flag = False

    def change_speed(self, value):
      #  print(f"Изменение скорости на {value}. Текущая скорость до изменения: {self.speed}")
        self.speed = max(0, min(self.speed + value, self.max_speed))
        print(f"Текущая скорость автобуса: {self.speed} км/ч после изменения")
    ...
    def seat_without_ticket_passenger(self, stop):
        ...
        for passenger_name, boarding_stop, exit_stop in self.without_ticket_passengers[:]:
            if boarding_stop == stop:
                print(f"Пассажир без билета {passenger_name} пытается сесть на остановке {stop}, "
                      f"ему надо доехать до остановки {exit_stop}")
    ...
    def show_bus(self, stop):
        """Графическое отображение состояния автобуса с размещением пассажиров по два в ряд."""
        fig, ax = plt.subplots()
        ax.set_xlim(0, 12)  # Увеличили ширину для двух сидений в одном ряду
        num_rows = (len(self.seats) + 1) // 2
        ax.set_ylim(0, num_rows + 1)
        ax.set_title(f'Остановка {stop}, состояние автобуса, текущая скорость: {self.speed} км/ч')

        for i, (seat, passenger) in enumerate(self.seats.items(), start=1):
            row = (i - 1) // 2 + 1  # Номер ряда
            col = (i - 1) % 2  # Позиция сидения в ряду (0 или 1)

            if seat in self.reserved_seats and passenger is None:
                color = 'blue'  # Зарезервировано
            elif passenger is not None:
                color = 'green'  # Занято пассажиром
            else:
                color = 'white'  # Свободно

            # Рисуем сидение с учетом позиции в ряду
            ax.add_patch(
                patches.Rectangle((1.5 + col * 5, num_rows - row + 1), 4, 0.8, edgecolor='black', facecolor=color))
            ax.text(3.5 + col * 5, num_rows - row + 1.4, f'{seat}: {passenger if passenger else "Свободно"}', ha='center',
                    va='center')
...
bus.add_without_ticket_passenger("Кузнецов", 2, 5)
bus.add_without_ticket_passenger("Панин", 2, 4)
